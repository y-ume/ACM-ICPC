...
void Pre(){
    ...
    rep(i,0,_){
        int c = q[i] , cur = dfs[c];
        if(top[c] == -1) top[c] = c , seg[c][0] = mp(dfs[c],dfs[c]);
        else Update(seg[top[c]][0] , mp(dfs[c],dfs[c]));
        int t = wson[c];if(~t) q[_++] = t , dfs[t] = cur + 1 ,
            cur += sz[t][3] , top[t] = top[c] , K[t][3] = sz[c][3] - sz[t][3];
        assert(~wson[c] || sz(g[c]) == 0);
...
void Modify(int x,ui d){
    ...
    while(x){
        int y = top[x];
        if(x != y) tree[2].Add(seg[y][0].fi , seg[y][0].se,dfs[y]+1,dfs[x],dd);
        x = par[y];if(y) f[x] += dd * (sz[x][3] - sz[y][3]);
    }
}
ui Query(int x){
    return f[x] + tree[0].Query(0,n-1,bfs[x] , 0) + tree[1].Query(0,n-1,bfs[x] , 0) +
        (~wson[x] ? tree[2].Query(seg[top[x]][0].fi, seg[top[x]][0].se , dfs[wson[x]] , 0) : 0);
}
